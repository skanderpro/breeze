...
class QuoteAverageTime extends AbstractSupplierReport
{
  ...
  private function calculateResults($query, $dateRange, $interval, $endDate)
  {
    return $dateRange->mapWithKeys(function ($date) use (
      ...
      $totalDays = $clonedQuery
        ->whereBetween("pos.created_at", [$date, $nextDate])
        ->whereNull("billable_date")
        ->get()
        ->map(function ($item) {
          return Carbon::parse($item->created_at)->diffInDays(Carbon::now());
        })
        ->sum();

      return [
        $date->format($interval === "1 day" ? "d/m" : "Y-m-d") => $totalDays,
      ];
    });
  }
}
